...
public:
    ...
    //std::vector<ICanvas::VertexRef> ref_table;
    unsigned int point;
...
class SelectableRef2 {
public:
    enum class Type { ENTITY, CONSTRAINT, SOLID_MODEL_EDGE, DOCUMENT };
    Type type;
    UUID item;
    
    ICanvas::VertexRef *ref_table;
    unsigned int point;
    bool is_entity() const
    {
        return type == Type::ENTITY;
    ...
    friend auto operator<=>(const SelectableRef2 &, const SelectableRef2 &) = default;
    friend bool operator==(const SelectableRef2 &, const SelectableRef2 &) = default;
};
